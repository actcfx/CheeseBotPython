...
class Deleted_mes(Cog_Extension):
    ...
    async def on_message_delete(self, message):
        ...
        DEL_MES_CHANNEL: nextcord.channel = self.bot.get_channel(DEL_MES_CHANNEL_ID)
        DEL_ATTACHMENT_CHANNEL_ID: int = ConfigData.load_data("config/channels.json").get("del_attachment_channel")
        DEL_ATTACHMENT_CHANNEL: nextcord.channel = self.bot.get_channel(DEL_ATTACHMENT_CHANNEL_ID)

        if message.guild.id != GUILD_ID:
            return

        embed = nextcord.Embed(title="Message Deleted", color=0xFB9966)
        ...
        del_mes = await DEL_MES_CHANNEL.send(embed=embed)
