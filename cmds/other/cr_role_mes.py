...
with open("config/roles.json", "r", encoding="utf-8") as role:
...
class Cr_role_mes(Cog_Extension):
    ...
    async def cr_WL(self, ctx):
        embed = nextcord.Embed(title="世界等級", description="領取你的世界等級", color=0x7B219F)
        embed.add_field(name="點擊下方表情符號即可領取對應身份組", value="", inline=False)
        embed.add_field(name="`發送求助前請先領取世界等級身份組`", value="", inline=False)
        embed.set_image(
    ...
    async def cr_server(self, ctx):
        embed = nextcord.Embed(title="遊戲伺服器", description="請選擇遊玩的伺服器", color=0x7B219F)
        embed.add_field(
            name="點擊下方表情符號即可領取對應身份組", value="🇦 ：北美，🇧 ：歐洲，🇨 ：亞洲，🇩 ：台港澳", inline=False
        )
        embed.add_field(name="`發送求助前請先領取遊戲伺服器身份組`", value="", inline=False)
        embed.set_image(
